...
coefs_svd_pinv = np.linalg.pinv(X_all) @ y_all
np.savetxt('coefs_svd_pinv.csv', coefs_svd_pinv, delimiter=',')

# rank deficient implies XTX inverse can't be found so svd won't work for a rank deficient matrix
...
# X_1 = X_all[:, 1]
# coefs_1 = np.linalg.inv(X_1.T @ X_1) @ X_1.T @ y_all
...
# plt.plot(X_feature, X_feature * coefs_1, color='red')
# plt.xlabel("Square Feet")
# plt.ylabel("Price")
# plt.title("Price vs Square Feet")
# plt.show()
# plt.savefig("Price_vs_SquareFeet.png")  


# Use X as only the square feet feature and y as the price to build a linear model
# calculate the predictions using the coefs
...
# two ways of regression: orthogonal line and minimizing the least squares